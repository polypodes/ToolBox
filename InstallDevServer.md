...
- hostname : `monClient`
login: `admin` (`root` créé par défaut)
mdp : `***********`
...
- Entre 23h et 6h du matin, on n’alloue plus que 1 CPU et 1 Giga de RAM au lieu de 4 CPU et 4 Giga de RAM le reste du temps.
...
Adding `monClient` as ServerName to Apache2 in `/etc/apache2/apache2.conf`
...
Configuration de PhpMyAdmin : Modification de lʼalias `phpmyadmin` en `myVerySecretPathToPhpMyadmin` dans le fichier `/etc/apache2/conf.d/phpmyadmin.conf`
...
Création dʼun utilisateur sql et dʼune base attachée : `monprojet_prod` / `monprojet_prod` / `***********`
